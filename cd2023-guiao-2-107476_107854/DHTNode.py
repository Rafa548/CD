...
class FingerTable:
    ...
    def __init__(self, node_id, node_addr, m_bits=10):
        ...
        for i in range(self.m_bits):
            self.table[i] = (node_id+2**i)%(2**self.m_bits)
        
        for i in range(self.m_bits):
            self.perma_table[i] = (node_id+2**i)%(2**self.m_bits)
    ...
    def find(self, identification):
        ...
        for x,y in self.finger_table.items():
            if contains(self.node_id, self.table.get(x),identification):
                if x-1 < 0:
                    return self.finger_table.get(0)[1]
                return self.finger_table.get(x-1)[1]
        

    def refresh(self):
        """ Retrieve finger table entries requiring refresh."""
        
        return [ (k+1,self.perma_table[k],v[1]) for k, v in self.finger_table.items()]


    def getIdxFromId(self, id):
        """ Get index of finger table entry corresponding to id."""
        for x,y in self.perma_table.items():
            if contains(self.node_id,y,id) :
                return x + 1
        return self.m_bits
    ...
    def as_list(self):
        ...
        """
        return [v for k, v in self.finger_table.items()]
...
class DHTNode(threading.Thread):
    ...
    def node_join(self, args):
        ...
        if self.identification == self.successor_id:  # I'm the only node in the DHT
            ...
            for i in self.finger_table.refresh():
                ...
                self.send(addr, {"method": "SUCESSOR", "args": {"id": i[1], "from": i[2]}})
    ...
    def get_successor(self, args):
        ...
        self.send(args["addr"], {"method": "SUCCESSOR_REP", "args": {"successor_id": self.successor_id, "successor_addr": self.successor_addr}})
        ...
        while not self.done:
            ...
            if payload is not None:
                ...
                if output["method"] == "JOIN_REQ":
                    ...
                    self.finger_table.update(output["args"]["index"], output["args"]["id"], output["args"]["addr"])
                    pass
